...
**Lane 1 — OPEN (felt leak + committed buyer).** At least one flagged finding survives both filters. Use a strong single finding as the opener (see selection below). Don't stack multiple findings in one email. This is the only lane that produces a cold send in this track — the send gate requires a verified finding.

**Bank everything that survived, not just the winner.** Every visually-confirmed finding that passes both filters goes into the lead's Findings Bank, **ranked by depth first (deep over shallow), then tier, then sting** (depth and tier are the two axes in `references/walk.md` — depth = self-fixability, tier = severity). #2 onward is what cold Touch 2/3 draws on (`crm-gate send --carries second-finding` checks the bank, so a discarded finding is a follow-up that can't happen). Unverified candidates and filter-fails never enter the bank — it holds openable findings only, just ranked.

**Why depth leads the ranking (the correction that matters most).** The most falsifiable finding is almost always the most trivial one — a dead link, a test SKU, a form that should be a scheduler. The coach fixes it herself in minutes and replies "thanks, fixed it," with no reason left to pay (Rita, Avneet, Lisa all did exactly this). So depth is now the first ranking key: a deep, un-self-fixable finding is preferred as the lead/anchor **even when a shallow finding is more provable.** Stop treating "most falsifiable" as "best."

**Bait-and-reserve (required on every Lane 1 lead).**
- The **opener** may be shallow — a specific, credible shallow finding earns the reply and opens the door (the Emily Ray move: a small fix opens, a bigger withheld finding gets her on a call). Choosing a shallow opener is fine.
- But the walk must **always identify at least one deep finding and hold exactly one in reserve as the reason for a call** — marked `RESERVED` in the bank, never given away in email. In the property this is the `RESERVED | DEEP | …` line; the send gate (`crm-gate send`) will never draw a RESERVED entry as a second-finding, so it stays held.
- **If no deep finding survives both filters, flag the lead low-value** (reply-likely, close-unlikely): put `low-value: no deep finding to reserve` as the FIRST line of `Notes`, note it in the page-body Findings Bank section, and say so in the confirm line. Do not pretend a shallow finding will close — it won't. The lead can still be worked for the reply, but it isn't a close candidate.
- Depth of what's given free is capped downstream (`haytham-email-draft/references/mechanics.md` free-value cap): shallow findings can be named freely, a deep finding's fix/diagnosis is call-only. The walk's job is to classify and reserve; the email skill enforces the cap.
...
Lane 1 only: state the chosen opener finding (bank #1) as the opening angle. One sentence, concrete, bespoke to this page, framed as felt cost. Falsifiability is a phrasing goal here — make the line checkable and specific — NOT the selection key: bank #1 is chosen by the depth-first ranking in Step 4, not by whichever finding is easiest to prove. This is what feeds into the email skill — the email skill takes this line and builds from it. Every other banked finding gets the same treatment in miniature (one felt-cost line + its innocent explanation) in the page body's Findings Bank section — it's follow-up material and deserves to be usable when Touch 2 needs it. The `RESERVED` deep finding gets named as a felt cost too, but its fix is held for the call, never written into the opener or any follow-up.
...
- **`Findings Bank` (property):** the ranked verified findings in compact machine-parseable lines, `N. STATUS | DEPTH | verified:YYYY-MM-DD | <finding>` — `STATUS` = `UNUSED` on a fresh walk (send-confirmation logging flips to `USED-TN` later) OR `RESERVED` for the one deep finding held as call bait; `DEPTH` = `SHALLOW` or `DEEP`; `verified:` = **today's date, stamped now, at walk time** (`refresh-finding` bumps it later). Example: `1. UNUSED | DEEP | verified:2026-07-27 | pricing split across 4 platforms` / `2. UNUSED | SHALLOW | verified:2026-07-27 | booking button drops to a form, not a scheduler` / `3. RESERVED | DEEP | verified:2026-07-27 | whole program readable free`. Ranked depth-first per Step 4. **Both tags are mandatory on every line, and rank 1 must be `DEEP`** — `crm-gate send` hard-fails a touch-1 opener drawing an untagged or `SHALLOW` entry, and hard-fails any line missing `verified:`. Omitting them does not degrade gracefully; it blocks the send. (Both were optional until 2026-07-27 and the result was 203 of 225 live entries with no depth tag and every freshly-walked row arriving already stale.) The send gate also skips `RESERVED` when drawing a second-finding. If no deep finding exists the lead has no legal opener: no `RESERVED` line, and the low-value flag goes in `Notes`. Lane 2/3: leave empty.
...
After writing, confirm to Haytham: lane verdict, the chosen opener finding in one line, the innocent explanation, how many findings were banked and their depth split (e.g. "banked 3 — 1 shallow opener, 1 deep second-finding, 1 deep RESERVED as call bait"), whether a deep finding is held in reserve (or the low-value flag if none), and whether `Finding Verified` was checked. That's enough for the email skill to draft a Lane 1 opener today. Mention that the SMYKM hook line was written as "not run yet" and that `haytham-hook-finder` must resolve it before the draft can be created.
