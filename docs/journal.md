...
**RULE LOCKED IN (Haytham, firm): never hold a due follow-up back as a "judgment call" — draft EVERY due one (warm/cold/ambiguous), Haytham reviews finished drafts.** Encoded into `uae-tick` SKILL.md step 3 (the run that drafts due follow-ups). NOTE: the rule is "always draft," NOT "draft the night before" — uae-tick is a morning routine; the night-before batch prep was THIS session's manual, on-request workflow, not something the tick does. (Christina had been wrongly surfaced un-drafted; that's what triggered it.)

**Christina Steinhoff → CONFIRMED OUT-OF-OFFICE AUTORESPONDER, parked to 07-25.** Her two "replies" were the same OOO auto-reply (identical wording, ~1-min latency, unfilled merge tags); Haytham's own WhatsApp test to the +971-562737368 on it got his number spam-flagged/blocked 6h (automated WhatsApp Business gate). So NOT warm: Sequence reverted Warm→Cold, Status Reply Received→Outreach Sent, Next Action → 2026-07-25 (cold Touch 3, day 9). The warm reply I'd drafted is cancelled — no Christina draft in Inbox 1 drafts OR the scheduled queue (verified). Cleaned her Notes property to match the (already-updated) body decision. **Do NOT retry WhatsApp — account-safety.**

### Open follow-ups
- [ ] Christina: Touch 3 email due 2026-07-25 (Inbox 1); bank #1+#2 spent so it carries loom-offer or the disambiguating question. If still no human reply after it → Dormant.
- [ ] uae-tick 07-20: Lisa carries a warm-bump Notes marker (log Touch 3 warm on departure, no bank spend); the 14 cold rows carry cold-T2 markers (log Touch 2, bank #2 → USED-T2). Do NOT re-draft any of them. Christina is NOT due 07-20 anymore (parked 07-25).
