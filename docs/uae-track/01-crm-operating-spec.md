...
>   funnel-fix offer. The live offer is The First Five: 2,000 AED setup,
>   then 900 AED per qualified call that
...
| `Top Program Price (AED)` | number | **Added 2026-07-28 with the re-niche** (live in Notion). Her HIGHEST live program, in AED (convert USD at the 3.6725 peg; anything else, use a live rate). Gate 0's fifth floor is AED 5,000. Blank = not yet resolved, which is the most common state (48 of 122 walked leads showed no price at all) and is NOT a pass. Never fill it with a guess or with her cheapest product — a AED 299 workshop next to AED 6,600 containers means 6600 goes here. |
...
| `Finding Type` | select | `No opt-in capture` `Weak/no nurture sequence` `Broken checkout` `No order bump/upsell` `Weak sales page` `No launch system` `Dead/stale element` `Broken booking flow` `No visible pricing` `Other` — descriptive only. It classifies the CALL BAIT; since 2026-07-27 the opener is a cold read and carries no finding. No new option needed. |
| `Findings Bank` | text | Every verified finding from the walk, ranked depth-first, one per line: `N. RESERVED \| DEPTH \| verified:YYYY-MM-DD \| finding`. **Since 2026-07-27 every finding is `RESERVED`** — the opener is a cold read, no email carries a finding, and nothing is spent by a send. They are call bait. `DEPTH` ∈ `SHALLOW` / `DEEP`; `verified:` is the date the finding was last confirmed true (set at walk time, bumped by `refresh-finding`). Rank 1 is the strongest and is what the `finding-verifier` certifies. Legacy rows still carry `UNUSED` / `USED-Tn` statuses and still parse — leave them. |
...
| `Gate 0 Failed Floors` | multi_select | Added 2026-07-27; `Program below AED 5000` added 2026-07-28. `Not UAE-based` `No funnel or paid offer` `Inactive 30d` `Audience below floor` `Program below AED 5000` (the last one is not yet a Notion option — it is created on the first write that uses it, deliberately, to avoid rewriting the option list on 246 live rows) — which of Gate 0's five floors actually fired (a lead can fail more than one). Set alongside `Gate 0 = Fail`, before moving to `Disqualified`. See `docs/uae-track/schema-delta.md` for the Airtable mapping and the spelling hazard vs. `Disqualification Reason` (different property, deliberately different wording — do not conflate). |
...
5. **The price never moves.** The First Five is 2,000 AED setup + 900 AED per qualified call, billing from call one (repriced 2026-07-28; `docs/uae-track/02-the-offer-first-five.md`). A low anchor is market data, not an instruction to discount. Objections get bonuses, restructured terms, or a named rung of the downsell ladder. The only other quotable number is 500 AED, The Named Fifty, and only on a live thread. Retired, do not quote: 735 / 2,575 / 3,600 AED, and the v1 First Five numbers 1,500 / 600.
